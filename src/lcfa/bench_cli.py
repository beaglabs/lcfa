...
def _load_factory(spec: str) -> Any:
    ...
    if not separator or not module_name or not attr: raise ValueError("subject factory must use module:callable syntax")
...
def _subject_from_args(args: argparse.Namespace) -> ReasonerSubject:
    ...
    if args.factory:
        ...
        if isinstance(engine, ReasonerSubject):
            if args.name and args.name != engine.name: return ReasonerSubject(name=args.name, reasoner=engine.reason, metadata=engine.metadata)
            return engine
...
def _builtins() -> dict[str, BenchmarkSuite]:
    suites = all_suites(); result = {s.id.removeprefix("lcfa-core-"): s for s in suites}; result.update({s.id: s for s in suites})
...
def _load_suite(spec: str) -> BenchmarkSuite:
    if spec.startswith("builtin:"):
        ...
        except KeyError as exc: raise ValueError(f"unknown built-in suite {name!r}; available: {', '.join(sorted(_builtins()))}") from exc
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--name"); run.add_argument("--profile", help="LCFA profile path for the built-in/base runtime")
    backend = run.add_mutually_exclusive_group(); backend.add_argument("--factory"); backend.add_argument("--artifact")
    run.add_argument("--backbone-path", help="override config.backbone.path for stochastic-flow artifacts")
    run.add_argument("--device-map", help="Transformers device_map override, for example auto or cpu")
    run.add_argument("--dtype", help="Transformers dtype override, for example auto, float16, bfloat16")
    run.add_argument("--repeats", type=int, default=3); run.add_argument("--warmup", type=int, default=0); run.add_argument("--output", "-o")
    compare = subparsers.add_parser("compare"); compare.add_argument("reports", nargs="+"); compare.add_argument("--baseline"); compare.add_argument("--output", "-o")
    listing = subparsers.add_parser("list"); listing.add_argument("--json", action="store_true")
    export = subparsers.add_parser("export"); export.add_argument("suite"); export.add_argument("--output", "-o", required=True)
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "run":
        ...
        if args.json:
            ...
            for suite in suites: print(f"{suite.id.removeprefix('lcfa-core-'):12} {len(suite.cases):>3} cases  {suite.id}")
